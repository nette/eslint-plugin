...
3. **Configs** (`src/configs/`): Configuration options for various environments:
   ...
   - `jsx.js`: JSX/TSX support — enables `ecmaFeatures.jsx` scoped to `.jsx`/`.tsx` files
...
6. **Tests** (`tests/`): Each rule has a corresponding test file using ESLint's `RuleTester`, plus integration tests:
   ...
   - Configuration tests: `javascript-config.test.js`, `typescript-config.test.js`, and `jsx-config.test.js` for testing configs
...
The plugin provides opt-in JSX/TSX support:

1. **JSX Config**: Available via `nette.configs.customize({ jsx: true })`
2. **File Scoping**: JSX parsing is scoped to `**/*.jsx` and `**/*.tsx` files only
3. **No Extra Dependencies**: Uses built-in `@stylistic/eslint-plugin` JSX support and ESLint's `ecmaFeatures.jsx`
4. **TypeScript Combo**: Works with `customize({ typescript: true, jsx: true })` from the `/typescript` entry point
5. **Test Coverage**: `tests/jsx-config.test.js` covers default-off, opt-in, file scoping, and TypeScript combination
